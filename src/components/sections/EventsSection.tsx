      ...
      <Container>
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <Reveal className="max-w-2xl">
            <SectionLabel>Eventos en vivo</SectionLabel>
            <Heading className="mt-5">
              La acción <span className="text-fire">nunca para</span>.
            </Heading>
          </Reveal>
          <Reveal delay={0.1}>
          ...
          {events.map((e, i) => (
            <StaggerItem key={e.name}>
              <article className="group relative flex h-full min-h-[18rem] flex-col justify-between overflow-hidden border border-ash-700 bg-ash-900 p-7 transition-colors duration-300 hover:border-ember/40">
                <div
                  aria-hidden
                  className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-ember/10 blur-2xl transition-opacity duration-500 group-hover:opacity-100 opacity-0"
                />
